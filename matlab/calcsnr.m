...
fs = 96000;
TSamp = 1/fs; 
t = (0:TSamp:2^21*TSamp)';
fsig = 3000; % Constructed to show some scallop loss
...
fbw = 6500;
ibw = round(fbw/fbin)+1;

% Plot and mark the above
loglog(f,Pxx);
hold on
itmp = (isig-5:isig+5)'; 
plot(f(itmp),Pxx(itmp),'r');
itmp = (ih3-4:ih3+4)'; 
plot(f(itmp),Pxx(itmp),'r');
itmp = (ih5-4:ih5+4)'; 
plot(f(itmp),Pxx(itmp),'r');
